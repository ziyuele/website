...
###########################################################################
	...
	mkdir -p /tmp/Master
	service_pid=/tmp/Master/master.pid
	function exit_with_usage(){
	echo "usage:"
	echo "\t--debug   # start debug moudle to start Master"
	...
	# start service
	nohup java  $JAVA_BASE_ARGV $JAVA_DEBUG_ARGV com.ziyue.website.master.Master $INPUT_ARGV >> nohup.out 2>&1 < /dev/null &
	pid=$!
